...
from torch_geometric.nn import GINConv  # Import GraphSAGE layer from PyTorch Geometric
...
class GNN_node(torch.nn.Module):
    ...
    def __init__(self, node_encoder, num_layer, input_dim, emb_dim, gnn_type='gin'):
        '''
        ...
        self.convs.append(GINConv(torch.nn.Sequential(torch.nn.Linear(input_dim, emb_dim), torch.nn.ReLU(), torch.nn.Linear(emb_dim, emb_dim))))
        self.batch_norms.append(torch.nn.BatchNorm1d(emb_dim))

        for layer in range(1, num_layer):
            self.convs.append(GINConv(torch.nn.Sequential(torch.nn.Linear(emb_dim, emb_dim), torch.nn.ReLU(), torch.nn.Linear(emb_dim, emb_dim))))
            self.batch_norms.append(torch.nn.BatchNorm1d(emb_dim))
