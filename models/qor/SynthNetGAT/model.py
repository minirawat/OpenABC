...
class GNN_node(torch.nn.Module):
    ...
    def __init__(self, node_encoder, num_layer, input_dim, emb_dim, gnn_type='gat', heads=1):
        '''
        ...
        self.convs.append(GATConv(input_dim, emb_dim, heads=heads, concat=True))
        self.batch_norms.append(torch.nn.BatchNorm1d(emb_dim * heads))

        for layer in range(1, num_layer):
            self.convs.append(GATConv(emb_dim * heads, emb_dim, heads=heads, concat=True))
            self.batch_norms.append(torch.nn.BatchNorm1d(emb_dim * heads))
...
class GNN(torch.nn.Module):
    ...
    def __init__(self, node_encoder, input_dim, num_layer = 2, emb_dim = 128,gnn_type = 'gat', heads = 1, graph_pooling = "mean"):
        super(GNN, self).__init__()
        self.num_layer = num_layer
        self.emb_dim = emb_dim
        self.graph_pooling = graph_pooling

        self.gnn_node = GNN_node(node_encoder,num_layer,input_dim,emb_dim,gnn_type = gnn_type, heads=heads)
...
class SynthNet(torch.nn.Module):
    ...
    def __init__(self, node_encoder, synth_encoder, n_classes, synth_input_dim, node_input_dim, gnn_embed_dim = 256,num_fc_layer=3, hidden_dim = 128, heads=1):
        ...
        self.gnn = GNN(self.node_encoder, self.node_enc_outdim+1, heads=heads)
