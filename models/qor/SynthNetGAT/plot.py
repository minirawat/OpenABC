...
def plotActualPredicted(df,xlabel,ylabel,leg_label,title):
    plt.style.use('ggplot')
    plt.rcParams['figure.figsize'] = [13,5]

    plt.plot([i+1 for i in range(len(df.prediction))], df.prediction, 'r')
    plt.plot([i+1 for i in range(len(df.actual))], df.actual, 'b')
    plt.show() 
    plt.savefig(osp.join(DUMP_DIR,'acutal_vs_pred.png'), format='png', bbox_inches='tight')

DUMP_DIR='/Users/minirawat/GitRepos/NETV1_baseline'
